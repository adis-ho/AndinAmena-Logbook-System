    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
                ...
                {/* Saldo Operasional (Driver only) */}
                {user?.role === 'driver' && (
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Saldo Operasional</label>
                        <div className="px-4 py-2.5 bg-green-50 border border-green-200 rounded-lg text-green-700 font-semibold">
                            {new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(user?.operational_balance || 0)}
                        </div>
                    </div>
                )}
            </div>
